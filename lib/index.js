...
exports.init = function (sequelize, optionsArg) {
  ...
  if (options.continuationNamespace) {
    ...
    if (!options.extraDataContinuationKey) {
      options.extraDataContinuationKey = 'extraData';
    }
  }
  ...
  function createBeforeHook(operation) {
    var beforeHook = function beforeHook(instance, opt) {
      ...
      if (options.extraDataFields) {
        // get all required field keys as an array
        var requiredFields = _.keys(_.pickBy(options.extraDataFields, function (required) {
          return required;
        }));
        if (requiredFields && requiredFields.length) {
          var extraData = ns && ns.get(options.extraDataContinuationKey) || opt['extraData'];
          var requiredFieldsProvided = _.filter(requiredFields, function(field) {
            return extraData[field] !== undefined;
          });
          if (requiredFieldsProvided.length !== requiredFields.length) {
            log('Required fields: ', options.extraDataFields, requiredFields);
            log('Required fields provided: ', extraData, requiredFieldsProvided);
    ...
    var afterHook = function afterHook(instance, opt) {
      ...
      if (debug) {
        ...
        if (ns) {
          ...
          log(`CLS ${options.extraDataFields}:`, ns.get(options.extraDataContinuationKey));
        }
      }
      ...
      if (instance.context && (instance.context.delta && instance.context.delta.length > 0 || destroyOperation)) {
        ...
        if (options.extraDataFields) {
          var extraData = ns && ns.get(options.extraDataContinuationKey) || opt['extraData'];
          if (extraData) {
            _.forEach(options.extraDataFields, function (required, field) {
              var value = extraData[field];
              if (debug) {
                log(`Adding extraData field to Revision - ${field} => ${value}`);
              }
